...
def sentence3():
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    A, B, C, D = PropSymbolExpr("PacmanAlive", 0), PropSymbolExpr("PacmanAlive", 1), PropSymbolExpr("PacmanBorn", 0), PropSymbolExpr("PacmanKilled", 0)
...
def atMostOne(literals):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    newLiterals = []
    # choose 2 of the set to combine
    combinations = itertools.combinations(literals, 2)
    # build the ~(s1 & s2) = ~s1 | ~s2
    for i in combinations:
        s = ~i[0] | ~i[1]
        newLiterals.append(s)
    # get A & B, where literals = [A, B]
    return conjoin(newLiterals)
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals):
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # it means there is a true and only one true
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # if a wall is at (x, y), then Pacman can not move to (x, y) at time is t
    ...
    for x, y in non_outer_wall_coords:
        s = PropSymbolExpr(pacman_str, x, y, t)
        wall_sentences.append(s)
    pacphysics_sentences.append(exactlyOne(wall_sentences))

    # Pacman takes exactly one of the four actions in DIRECTIONS at timestep t.
    direciton_sentences = []
    for dir in DIRECTIONS:
        s = PropSymbolExpr(dir, t)
        direciton_sentences.append(s)
    pacphysics_sentences.append(exactlyOne(direciton_sentences))
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem):
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            wall_sentences.append(PropSymbolExpr(pacman_str, x, y, t))
        KB.append(exactlyOne(wall_sentences))
        ...
        direciton_sentences = []
        for dir in actions:
            direciton_sentences.append(PropSymbolExpr(dir, t))
        KB.append(exactlyOne(direciton_sentences))
