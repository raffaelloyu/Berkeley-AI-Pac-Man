...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        optimumAction = ''
        # get the most possible ghost position
        for ghost in livingGhostPositionDistributions:
            maxKey = max(ghost, key=ghost.get)  
            maxBeliefPositions.append(maxKey)

        # base on the ghost position calculate the minimum distance and choose the action
        for pisition in maxBeliefPositions:
            for legalAction in legal:
                successorPosition = Actions.getSuccessor(pacmanPosition, legalAction)
                distance = self.distancer.getDistance(successorPosition, pisition)
                if distance < minDistance:
                    minDistance = distance
                    optimumAction = legalAction
